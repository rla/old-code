...
term([( fx,Os)|Ops],T) --> ( oper(O,Os), term(Ops,E),            { T = compound(O,[E]) }
                           ; term(Ops,T) ).
term([( fy,Os)|Ops],T) --> ( oper(O,Os), term([( fy,Os)|Ops],E), { T = compound(O,[E]) }
                           ; term(Ops,T) ).
...
parse_terms(Result) --> ( term(T) -> parse_terms(Ts), { Result = [T|Ts] }
                        ; { Result = [] } ).
