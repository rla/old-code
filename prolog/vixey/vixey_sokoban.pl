...
sokoban :- initial_state(State), game_loop(0, [], State,_).
game_loop(D, Ms) --> display, {get_key(Direction)}, (move(Direction), {dir_pretty(Direction, M)} ; {true}), (win -> {write('You have won!'), nl, write(D), write(' moves:'), reverse([M|Ms], Ms1), write(Ms1)} ; {D1 is D + 1}, game_loop(D1, [M|Ms])).
 
get_key(K) :- get_single_char(C), char_code(X, C), (X = e -> halt
                       ; (direction(X,_,_) -> K=X, dir_pretty(K, T), write(T), nl ; get_key(K))
                       ).
...
dir_pretty(K,T):- member(K-T, [u-up, d-down, l-left, r-right]).
